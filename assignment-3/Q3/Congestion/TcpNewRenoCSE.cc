...
TcpNewRenoCSE::SlowStart (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
  ...
  if (segmentsAcked >= 1)
    {
      tcb->m_cWnd +=  static_cast<double>(pow(tcb->m_segmentSize,1.9) / tcb->m_cWnd.Get ());
  ...
  if (segmentsAcked > 0)
    {
      ...
      double adder =0.5*tcb->m_segmentSize; // static_cast<double>(pow(tcb->m_segmentSize,1.9) / tcb->m_cWnd.Get ());
